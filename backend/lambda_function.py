...
def handle_get_tasks():
    response = table.scan()
    return json_response(200, response["Items"])
...
def handle_post_tasks(body):
    if body.get("op") == "setStarred":
        task_id = body.get("taskId")
        if not task_id or "starred" not in body:
            return error_response(400, "taskId and starred are required")
        table.update_item(
            Key={"taskId": task_id},
            UpdateExpression="SET starred = :s",
            ExpressionAttributeValues={":s": bool(body["starred"])},
        )
    ...
    if body.get("op") == "setCompleted":
        ...
        if not task_id or "completed" not in body:
            return error_response(400, "taskId and completed are required")
        table.update_item(
            Key={"taskId": task_id},
            UpdateExpression="SET completed = :c",
            ExpressionAttributeValues={":c": bool(body["completed"])},
        )
        return json_response(200, {"message": "Task updated"})
    ...
    return json_response(200, {"message": "Task created"})


def handle_patch_task(task_id, body):
    if "completed" not in body:
        return error_response(400, "completed is required")
    table.update_item(
        Key={"taskId": task_id},
        UpdateExpression="SET completed = :c",
        ExpressionAttributeValues={":c": bool(body["completed"])},
...
def lambda_handler(event, context):
    ...
    if method == "PATCH" and path.startswith("/tasks/"):
        ...
        if body is None:
            return error_response(400, "Invalid JSON body")
        return handle_patch_task(task_id, body)
